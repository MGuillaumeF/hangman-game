...
CRUDOrderDispatcher::route(const std::string &objectType,
  ...
  if ("user" == objectType) {
    ...
    response = routeObjectType<user>(properties.get<std::string>("order-type"), properties, data);
  } else if ("word" == objectType) {
    // dispatch crud word orders
    response = routeObjectType<word>(properties.get<std::string>("order-type"), properties, data);
  } else {
