...
UserDBEndpoint::createUser(const boost::property_tree::ptree &data) const {
  ...
  for (user userToPersist : users) {
    if (0 != userToPersist.getId()) {
      std::cerr << "[WARNNING] create content ignored : the id must be 0, value " << userToPersist.getId() << " ignored" << std::endl;
    }
    if (0 != userToPersist.getVersion()) {
      std::cerr << "[WARNNING] create content ignored : the version will be overrided to 1, value " << userToPersist.getVersion() << " ignored" << std::endl;
    }
